...
1. I currently set low-density MIPS value to be 0.15 for 2D and 0.45 for 3D, but can you confirm if they are those?

2. For 3D, the liquid slab already depins from the wall at $\epsilon_w = 2$ (also the time evolution of A(t) is fluctuating so hard, see the plot.), but in your paper, it happens when it's less than $\epsilon_w = 2$. Do you think it just happened to be this case since I only had one realisation?